...
def draw_world_origin_axes(frame, K, T_camera_world, axis_len=0.2):
    ...
    if x_uv is not None:
        ...
        cv2.putText(frame, "X", x_uv, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)

    if y_uv is not None:
        cv2.line(frame, o_uv, y_uv, (0, 255, 0), 2)
        cv2.putText(frame, "Y", y_uv, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)

    if z_uv is not None:
        cv2.line(frame, o_uv, z_uv, (255, 0, 0), 2)
        cv2.putText(frame, "Z", z_uv, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1, cv2.LINE_AA)
...
def draw_line_with_global_text(frame, K, origin_cam, pos_cam, pos_world):
    ...
    # 빨간 선
    cv2.line(frame, o_uv, p_uv, (63, 127, 256), 2)
...
def main():
    ...
    K_orig, dist_coeffs, camera_params_raw, calib_image_size = load_camera_params(CALIB_JSON_PATH)
    ...
    while True:
        ...
        draw_world_origin_axes(frame, K_for_projection, T_camera_world, axis_len=0.2)

        # object / target에 대해 원점→태그까지 선 + Global 텍스트
        if obj_cam is not None:
            obj_world = R_wc @ obj_cam + t_wc
            draw_line_with_global_text(frame, K_for_projection, origin_cam, obj_cam, obj_world)

        if tgt_cam is not None:
            tgt_world = R_wc @ tgt_cam + t_wc
            draw_line_with_global_text(frame, K_for_projection, origin_cam, tgt_cam, tgt_world)
