...
CALIB_JSON_PATH = "./../data/D435_intrinsic_1280_720.json"
...
# 4) 카메라 인덱스
# 카메라 불러오기 안되면 바꿔보셈. 0, 1, 2 중에 하나일듯?
CAMERA_INDEX = 1
...
def load_camera_params(json_path):
    ...
    # pupil_apriltags Detector에 넣어줄 카메라 파라미터 튜플 (초기값)
    ...
    if "image_size" in data:
        w, h = data["image_size"]
        calib_image_size = (int(w), int(h))
        print(f"[INFO] 보정에 사용된 이미지 해상도: {w} x {h}")

    return K, dist_coeffs, camera_params, calib_image_size
...
def main():
    ...
    K_orig, dist_coeffs, camera_params, calib_image_size = load_camera_params(CALIB_JSON_PATH)
    print("[INFO] K (카메라 내부 행렬, 원본):")
    print(K_orig)
    ...
    if calib_image_size is not None and (w != calib_image_size[0] or h != calib_image_size[1]):
        print("[WARN] 실제 카메라 해상도가 보정에 사용된 해상도와 다릅니다. "
              "Intrinsic과 정확히 일치하지 않을 수 있습니다.")

    # --- Optimal new camera matrix 계산 ------------------------------------
    # new_K: undistort 이후 사용할 "가상 카메라"의 내부 파라미터
    new_K, _ = cv2.getOptimalNewCameraMatrix(K_orig, dist_coeffs, (w, h), 1)
    print("[INFO] getOptimalNewCameraMatrix로 얻은 새 K:")
    print(new_K)

    # AprilTag용 camera_params도 new_K 기준으로 갱신
    new_fx = float(new_K[0, 0])
    new_fy = float(new_K[1, 1])
    new_cx = float(new_K[0, 2])
    new_cy = float(new_K[1, 2])
    camera_params_new = (new_fx, new_fy, new_cx, new_cy)

    # project_point에서도 new_K를 사용하도록 K를 교체
    K_for_projection = new_K
    ...
    while True:
        ...
        # 1) 왜곡 보정 (undistort)
        #    - 입력: 원래 보정된 K_orig, dist_coeffs
        #    - 출력: new_K 기준의 undistorted image
        frame = cv2.undistort(frame_raw, K_orig, dist_coeffs, None, new_K)
        ...
        detections = detector.detect(
            ...
            camera_params=camera_params_new,  # new_K 기준 intrinsics
            tag_size=TAG_SIZE_M,
        )
        ...
        if len(detections) == 0:
            ...
            for det in detections:
                ...
                o_uv = project_point(K_for_projection, origin_cam)
                x_uv = project_point(K_for_projection, x_axis_cam)
                y_uv = project_point(K_for_projection, y_axis_cam)
                z_uv = project_point(K_for_projection, z_axis_cam)
        ...
        if ADJUST_WIN_SIZE is not None and ADJUST_WIN_SIZE > 0:
            ...
            new_w = int(w_disp * ADJUST_WIN_SIZE)
            new_h = int(h_disp * ADJUST_WIN_SIZE)
